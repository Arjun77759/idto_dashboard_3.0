...
import { resendPasswordReset } from '../api/authApi'
import { useToast } from '@/hooks/use-toast'

const ResetPasswordCheckInboxPage = () => {
    ...
    try {
      ...
      if (response.status === 'already_set') {
        toast({
          ...
          description: "This account already has a password set. You can proceed to login.",
        })
      ...
      const message = err?.response?.data?.detail || err?.message || 'Failed to resend reset link'
      toast({
    ...
    >
      ...
      <div className="flex flex-col gap-6 items-center p-2 rounded w-full max-w-[397px]">
        ...
        <p className="font-semibold leading-[1.5] text-[16px] text-[#616675] text-center tracking-[-0.16px] w-full">
          Click the link in your email and follow the instructions to reset your password.
        </p>
...
export default ResetPasswordCheckInboxPage
